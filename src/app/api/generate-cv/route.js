...
// ============================================================
// 🏛️ ARCHITECTURE TEMPLATE GENERATORS
// ============================================================
...
// Helper function to render if exists
...
function generateModernTemplate(data, industry) {
  ...
  const layouts = [
          ...
          <div class="section">
            ...
            ${projects.map(project => `
              <div style="margin-bottom: 20px;">
                ...
                ${project.link ? `<div style="font-size: 14px; color: var(--primary);">${project.link}</div>` : ''}
    ...
    // Layout 4: Minimalist with accent blocks
    () => {
      return `
      <!DOCTYPE html>
      <html>
...
function generateCreativeTemplate(data, industry) {
  ...
  const creativeType = Math.floor(Math.random() * 3);
  
  const creativeLayouts = [
    // Layout 1: Asymmetrical Creative
    () => {
      return `
      <!DOCTYPE html>
      <html>
            ...
            <div class="creative-block">
              ...
              ${projects.map(project => `
                <div style="margin-bottom: 20px;">
                  ...
                  ${project.description ? `<div style="font-size: 14px;">${project.description}</div>` : ''}
                </div>
              `).join('')}
            </div>
            ` : ''}
        ...
        <div class="magazine-container">
          ...
          <div class="magazine-section">
            ...
            <div class="contact-info">
              ${personalInfo.email ? `<div class="contact-item">📧 ${personalInfo.email}</div>` : ''}
              ${personalInfo.phone ? `<div class="contact-item">📱 ${personalInfo.phone}</div>` : ''}
              ${personalInfo.city ? `<div class="contact-item">📍 ${personalInfo.city}</div>` : ''}
              ${personalInfo.linkedin ? `<div class="contact-item">🔗 ${personalInfo.linkedin}</div>` : ''}
            </div>
          </div>
          ` : ''}
  ...
  return creativeLayouts[creativeType]();
}
...
function generateExecutiveTemplate(data, industry) {
  ...
  const executiveType = Math.floor(Math.random() * 3);
  
  const executiveLayouts = [
    // Layout 1: Corporate Executive
    () => {
      return `
      <!DOCTYPE html>
      <html>
            ...
            <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 40px;">
              ...
              <div class="executive-section">
                ...
                <div style="display: flex; flex-wrap: wrap; gap: 10px;">
                  ${skills.slice(0, 8).map((skill) => `
                    <div style="background: #f0f4f8; padding: 8px 16px; border-radius: 4px; font-size: 14px; font-weight: 500;">${skill}</div>
                  `).join('')}
                </div>
              </div>
              ` : ''}
          ...
          <div class="boardroom-columns">
            ...
            <div>
              ...
              <div class="boardroom-section">
                ...
                <div style="display: flex; flex-wrap: wrap; gap: 8px;">
                  ${skills.slice(0, 6).map((skill) => `
                    <span style="background: #f0f4f8; padding: 6px 12px; border-radius: 4px; font-size: 14px;">${skill}</span>
                  `).join('')}
                </div>
              </div>
              ` : ''}
  ...
  return executiveLayouts[executiveType]();
}
...
export async function POST(request) {
  try {
    ...
    const { formData, cvType, userId, industry } = body;
    ...
    if (!userData?.isPro && (userData?.tokens || 0) <= 0) {
      return NextResponse.json({ success: false, message: 'Insufficient credits.' }, { status: 403 });
    }
